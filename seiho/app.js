/* ============================================================
   変額・外貨建 過去問道場
   - 問題データは data/*.js が定義する SEIHO_HENGAKU_MOSHI / SEIHO_HENGAKU_EX /
     SEIHO_GAIKA_MOSHI / SEIHO_GAIKA_EX（各ファイルが分野1つ: subject → units → questions）を読む。
     ファイルが無い（404）分野はタブを作らない
   - 各分野の末尾に、全単元を連結した仮想単元「全部通し」を置く（履歴は元単元と共有）
   - 問題形式: t:"ox"（〇×） / t:"mc"（多肢選択 2〜10択, c:選択肢配列, a:正解index）
     任意で img（問題文の下に出す画像の相対パス）、topic（表示しない）
   - 解答履歴は localStorage("seiho-dojo-v1") に保存
   ============================================================ */
(() => {
"use strict";
...
/* ---------- 問題データの取り込み ---------- */
const SUBJECTS = [
  window.SEIHO_HENGAKU_MOSHI, window.SEIHO_HENGAKU_EX,
  window.SEIHO_GAIKA_MOSHI, window.SEIHO_GAIKA_EX,
].filter(s => s && Array.isArray(s.units));
SUBJECTS.forEach(s => {
  s.units = s.units.filter(u => u && u.id);
  s.units.forEach(u => { if (!Array.isArray(u.questions)) u.questions = []; });
});
  ...
  s.units.forEach(u => {
    ...
    u.questions.forEach((item, i) => {
      const q = {
        ...
        a: item.a,
        calc: !!item.calc,
        img: typeof item.img === "string" && item.img.trim() ? item.img.trim() : null,
...
// 各分野の「全部通し」: 全単元の問題を連結した仮想単元（問題・履歴は元の単元と共有）
SUBJECTS.forEach(s => {
  const all = [];
  ...
  s.units.push({
    id: "all", virtual: true,
    name: `全部通し（${all.length}問）`, key: `${s.subject}/all`, label: `【${s.name}】全部通し`,
    subject: s.subject, subjectName: s.name,
    questions: all,
  });
});
...
const SYNC_URL = String(window.SEIHO_SYNC_URL || "").trim();
const GUEST_KEY = "seiho-dojo-v1";
const ACCOUNT_KEY = "seiho-dojo-account";
...
function lsKey() { return account ? `seiho-dojo-user-${account.id}` : GUEST_KEY; }
...
// 計算問題が1問もなければ「計算問題」フィルタを出さない
if (!BANK.some(q => q.calc)) { const cb = $("#optRangeCalc"); if (cb) cb.remove(); }
...
function renderShuffleTree() {
  ...
  SUBJECTS.forEach(s => {
    ...
    const unitChks = [];
    s.units.forEach(u => {
      if (u.virtual) return;   // 仮想単元（全部通しなど）は他単元と重複するためツリーには出さない
...
// 分野ごとの「ランダム10問」: その分野の全単元を範囲・フィルタ「すべて」・10問でシャッフル演習
// （シャッフル欄のチェック状態・フィルタ設定は変更しない）
$("#btnRand10").addEventListener("click", () => {
  const s = SUBJECTS.find(x => x.subject === activeSubj);
  if (!s) return;
  const pool = shuffle(BANK.filter(q => q.subject === s.subject)).slice(0, 10);
  if (pool.length === 0) return;
  startQuiz(pool, `${s.short || s.name} ランダム10問`, {});
});
...
const elQText = $("#qText"), elQImg = $("#qImg"), answerArea = $("#answerArea");

// 問題の図（img がある問題だけ <img> を置く。無ければ中身を空にする）
function makeQImg(q) {
...
function renderQImg(box, q) {
  box.innerHTML = "";
  box.hidden = !q.img;
  if (q.img) box.append(makeQImg(q));
}
...
function renderQuestion() {
  ...
  elQCat.textContent = quiz.unit ? `${q.subjectShort} ${q.unit.name} 問${q.no}` : `${q.subjectShort}・${q.unit.name} 問${q.no}`;
...
function showResult() {
  ...
  results.forEach((r, i) => {
    ...
    if (r.q.img) {
      ...
      fig.className = "qimg-box rv-img";
      fig.append(makeQImg(r.q));
      details.append(fig);
    }
    details.append(exp);
    li.append(details);
...
/* ============================================================
   成績（分野ごとの得点率＋単元内訳）
   ============================================================ */
/* 成績の描画は「subjects と hist を受け取る」形にしている。
...
function renderWeekly() {
  ...
  for (const [k, v] of Object.entries(store.hist)) {
    ...
    // 同期アカウントを別アプリ（同じGASを使う過去問道場）と共用しているため、他アプリの分野の記録は集計しない
  ...
  [...weeks.keys()].sort((a, b) => b - a).slice(0, 12).forEach(wk => {
    ...
    row.querySelector(".week-subj").textContent = SUBJECTS
      ...
      .map(s => {
        ...
        return `${s.short || s.name}${Math.round((b.ok / b.att) * 100)}%（${b.att}）`;
      }).join("　");
    body.append(row);
  });
}
...
$("#btnResetHist").addEventListener("click", () => {
  if (confirm("このアプリ（変額・外貨建）の解答履歴（正誤・チェック）をすべて削除します。よろしいですか？")) {
