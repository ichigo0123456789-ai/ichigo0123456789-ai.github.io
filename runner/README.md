...
| `lib/k109.js` / `lib/toho.js` / `lib/cinecitta.js` / `lib/sunshine.js` | 109シネマズ / TOHOシネマズ(vit) / チネチッタ(SMART THEATER) / シネマサンシャイン(cinerino+COA) の各アダプタ。同じメソッド面 |
...
| `gdcs` | グランドシネマサンシャイン 池袋 | **シネマサンシャイン（cinerino＋COA・`lib/sunshine.js`）**。ゲスト購入。確保は accept→authorize の2段で COA 連携のため完了まで数秒（受付は即時）。取引状態を決済ブラウザに注入し券種選択から人間が続ける（券種→次へ→購入者情報入力まで動作確認済み）。12スクリーン・2,407席（実データ 2026-08-23）。一般発売は上映2日前 0:00／会員は3日前 21:00（番組表に発売日時が入っている） |
...
**劇場追加は `node runner/add-theater.js <spec> <key> "<名前>" "<エリア>" "<都道府県>" [--chain kinezo|109|toho|cinecitta|sunshine]` で自動**（実サイトから全スクリーンの座席を採取し、theaters.js / runner を一括更新。KINEZO は劇場パス、109 はサイト別名（例 `I1`）を spec に渡す）。
未対応：新文芸坐（eigaland・別アダプタで対応予定）。

**規約同意について**：チネチッタ／シネマサンシャインは座席画面に「利用規約に同意」チェック（UIのみのゲート）があります。runner は API で確保するためこのチェックを通りません。**各サイトの利用規約に同意したうえで**使ってください（runner 起動時にも注意書きを表示します）。
...
## 席を選ばず「座席選択画面まで最速で開く」モード（`--open-seatmap`）

事前に席を決めず、**発売の瞬間に座席選択画面をブラウザで開いて、そこで自分で席を選びたい**とき用。
予約導線の確定・座席画面セッションの確立までを高速HTTPで済ませ、実ブラウザに引き継ぎます。
`--seats` は不要。発売時刻に合わせるなら `--at` を併用。

```bash
node runner/reserve-hybrid.js --date 2026-08-27 --title ユーフォニアム --time 10:00 \
  --open-seatmap --at "2026-08-25T00:00:00+09:00"
```

- 既存の「席を指定して自動確保」モード（`--seats`）と併用はしません（`--open-seatmap` を付けた場合はそちら優先）。
- 席・券種・決済は自動化せず、開いた画面で人間が選びます。
