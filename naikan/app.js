...
/* ---------- 問題データの取り込み ---------- */
const SUBJECTS = [
  window.NAIKAN_CH1, window.NAIKAN_CH2, window.NAIKAN_CH3, window.NAIKAN_CH4,
  window.NAIKAN_CH5, window.NAIKAN_CH6, window.NAIKAN_CH7, window.NAIKAN_CH8,
  window.NAIKAN_LEVEL,
].filter(Boolean);
...
const SYNC_URL = String(window.NAIKAN_SYNC_URL || "").trim();
const GUEST_KEY = "naikan-dojo-v1";
const ACCOUNT_KEY = "naikan-dojo-account";
...
function lsKey() { return account ? `naikan-dojo-user-${account.id}` : GUEST_KEY; }
...
// 計算問題が1問もなければ「計算問題」フィルタを出さない（内管責には計算問題がない）
...
// 章ごとの「ランダム10問」: その章の全単元を範囲・フィルタ「すべて」・10問でシャッフル演習
