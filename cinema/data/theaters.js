...
(function () {
  ...
  var CHAINS = {
    tjoy: {
      ...
      /* ---- 予約ルール（サイトごとに異なるので、チェーン単位でここに集約する）----
         UI はこのオブジェクトだけを参照する。別チェーンを追加するときも
         同じ形で持たせれば、枚数上限や注意書きが自動で切り替わる。 */
      rules: {
        /* 1回の予約で選択できる座席数の上限（KINEZO 公式FAQより） */
        maxSeats: 5,
        /* 「選択席の間に1席だけ空きを残す配置」の禁止。
           KINEZO の公式ルールとしては確認できていないため強制はせず、
           該当する配置を選んだときに注意だけ出す。 */
        singleGap: { enforce: false, warn: true,
          note: '席と席の間に1席だけ空きを残す配置は、劇場によっては選択できません（KINEZO での扱いは未確認）' },
        notes: [
          '一度に予約できる座席は最大5席。6枚以上は申込みを複数回に分ける',
          '同一アカウント・同一上映回の「あとから決済」は合計10枚まで',
